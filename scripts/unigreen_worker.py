...
import os
import io
import pypdf  # Il successore di PyPDF2
...
if GEMINI_KEY:
    ...
    model = genai.GenerativeModel('gemini-1.5-flash') # Gemini 3 non è ancora pubblico via API standard, Flash è il top ora.
...
PROFILO_UTENTE = "Studente di Economia (Dipartimento Biagi). Interessi: Mobilità internazionale, BIP, Erasmus, Borse di studio, Finanziamenti. Escludi: Giurisprudenza, Medicina."
    ...
    try:
        ...
        if url.lower().endswith(".pdf") or "application/pdf" in response.headers.get('Content-Type', ''):
            with io.BytesIO(response.content) as f:
                ...
                testo = ""
                # Leggiamo le prime 15 e le ultime 15 (Strategia Claude)
...
def analizza_con_ai(testo):
    ...
    except: return "Errore", "Errore", "Errore", False, "0"
